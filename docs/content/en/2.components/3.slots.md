...
::alert{type="warning"}
Other toolbar tools (`search`, `rotate`, `openFile`, `print`, `download`, `fullscreen`, `themeToggle`, `moreOptions`) do not expose named slots in the current version. To customize them, hide the built-in tool via the [`toolbar` prop](/en/components/props) and provide your own control next to the viewer.
::
