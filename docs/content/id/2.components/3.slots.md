...
::alert{type="warning"}
Tool toolbar lainnya (`search`, `rotate`, `openFile`, `print`, `download`, `fullscreen`, `themeToggle`, `moreOptions`) tidak menyediakan slot bernama di versi saat ini. Untuk mengkustomisasinya, sembunyikan tool bawaan melalui prop [`toolbar`](/id/components/props) dan sediakan kontrol sendiri di samping viewer.
::
